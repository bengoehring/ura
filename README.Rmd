...
The `rater_agreement()` function is the key method for monitoring the accuracy and progress of raters. While `irr_stats()` provides pooled IRR statistics across all raters, `rater_agreement()` provides the percent share of a given raters' codings that agree with other raters' codings. In other words, it offers supervisors a method for checking the relative accuracy of each rater in real time. Since interventions in coding procedures should be used sparingly, I suggest taking a look at the working paper linked above for more information about when and why to intervene based on information gleaned from `rater_agreement()`: 
